...
HWC3::Error HWCDisplay::SetLayerType(LayerId layer_id, LayerType type) {
  const auto map_layer = layer_map_.find(layer_id);
  if (map_layer == layer_map_.end()) {
    DLOGW("display [%" PRIu64 "]-[%d] SetLayerType (%" PRIu64 ") failed to find layer", id_,
          static_cast<int>(type_), layer_id);
    return HWC3::Error::BadLayer;
  }
...
void HWCDisplay::DumpInputBuffers() {
  ...
  for (uint32_t i = 0; i < layer_stack_.layers.size(); i++) {
    ...
    if (!handle) {
      DLOGW(
          "Buffer handle is detected as null for layer: %s(%" PRIu64 ") out of %zu layers with "
          "layer "
          "flag value: %u",
